(function () {
    ...
    var now = new Date();
    var i, arrivalDate, departureDate, prevDepartureDate;

    for (i = 0; i < arrivals.length; i += 1) {
        arrivalDate = new Date(arrivals[i].attributes[0].value);
        departureDate = new Date(departures[i].attributes[0].value);
        prevDepartureDate = new Date(departures[i + 1].attributes[0].value);
    ...
    */
    ...
    function showProgress(i, position, startDate, finishDate) {
        var voyageProgress =
                Math.round((1 - (finishDate - now) / (finishDate - startDate)) *
                100);
        var progressBarMin;
        var rowProgress;
        var positionEng;

        (position == "Стоянка в порту") ? (positionEng = "At port")
                : (positionEng = "Seapassage");
        ...
        rowProgress.innerHTML = '<td><span class="lang lang-ru">' +
                position + '</span><span class="lang lang-en hide">' +
                positionEng + '</span></td><td colspan="4">' +
                '<div class="progress-bar progress-bar-success"' +
                ' role="progressbar" aria-valuenow="' +
                voyageProgress +
                '" aria-valuemin="0" aria-valuemax="100" style="width:'
                + progressBarMin + '%;">' +
                voyageProgress + '%</div></td>';
